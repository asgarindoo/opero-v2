...
export default function ChatEmptyState() {
  const { channels, loadingChannels, error } = useChat();
  ...
  }, [channels, loadingChannels, router]);
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-surface">
      ...
      <p className="font-body-sm text-[13px] text-on-surface-variant max-w-sm mb-6 leading-relaxed opacity-70">
        {loadingChannels ? "Loading workspace channels..." : error ?? "Select an active channel from the sidebar or start a new one to begin collaborating seamlessly."}
      </p>
    </div>
  );
}
