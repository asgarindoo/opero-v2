...
export async function listGoals() {
  ...
  const goals = await prisma.goal.findMany({ where: { organizationId: ctx.tenantId }, orderBy: { createdAt: "desc" }, include: { createdBy: { select: { id: true, name: true, image: true } } } });
  return goals.map((goal) => mapDomainRecord(goal));
}

export async function getGoalById(id: string) {
  const ctx = await requireTenant();
  const goal = await prisma.goal.findFirst({ where: { id, organizationId: ctx.tenantId }, include: { createdBy: { select: { id: true, name: true, image: true } } } });
...
export async function updateGoal(id: string, patch: Record<string, unknown>) {
  ...
  const updated = await prisma.goal.findFirst({ where: { id, organizationId: ctx.tenantId }, include: { createdBy: { select: { id: true, name: true, image: true } } } });
